...
class ChromaVectorStoreAdapter(VectorStorePort):
	...
	def __post_init__(self) -> None:  # pragma: no cover - lazy import path exercised in tests indirectly
		try:
	...
	def search(self, query_vector: Sequence[float], top_k: int = 5) -> List[RetrievedChunk]:
		...
		for i in range(len(ids)):
			out.append(
				RetrievedChunk(
					id=str(ids[i]), text=docs[i] or "", score=float(dists[i]), metadata=metas[i] or {}
				)
			)
		return out
